...
# 爬取角色头像
def get_characters_avatar(character_list=None, verbose=False):
    ...
    for index, character in enumerate(character_list):
        ...
        soup = BeautifulSoup(response.text, "lxml")
        image_url = soup.select('.img-kk')[0]["src"]

        avatar_save_path = os.path.join(save_path, "%d.png" % index)
        result[character["name"]] = {
            "id": index,
            "character_name": character["name"],
            "avatar_url": image_url,
            "avatar_path": "%d.png" % index
        }
        urllib.request.urlretrieve(image_url, avatar_save_path)

    with open(os.path.join(save_path, "avatars.json"), "w+") as file:
        file.write(json.dumps(result, indent=4, ensure_ascii=False))
